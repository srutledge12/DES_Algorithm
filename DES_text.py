...
def encrypt(messageFile, keyFile, outFile):
    ...
    while (bv.more_to_read):
        ...
        while(bitvec._getsize() < 64):
            # print(type(bitvec._getsize()))
            bitvec += BitVector(bitstring = '0')
            
        for round in round_key:
            # print(round)
            if bitvec._getsize() > 0:
                ...
                # out_xor = newRE
                out_xor = newRE ^ ( round )
                RE_Temp = substitute(out_xor)
                RE_modified = RE_Temp.permute( pbox_permutation )
                newRE = (RE_modified ^ LE)
                
                final_string = RE + newRE 
                retString = newRE + RE
                bitvec = final_string
        # break
        print(retString.get_bitvector_in_hex())
        f.write(retString.get_bitvector_in_hex())
    f.close()
...
def decryption(encryptedFile, keyFile, outFile):
    ...
    # bv = BitVector(filename = encryptedFile)
    # print(bv._getsize())
    roundInd = 0
    f = open(outFile, "w")
    # while (bv.more_to_read):
    ...
    #         bitvec += BitVector(bitstring = '0')
    mult = 1
    while(mult * 64 - 1 < bv._getsize()):
        roundRev = len(round_key)-1    
        bitvec = bv[64*(mult-1): 64*mult]
        # print(bitvec)
        while(roundRev >= 0):
            
            # print(round_key[roundRev])
            if bitvec._getsize() > 0:
                [LE, RE] = bitvec.divide_into_two()
                # print(RE._getsize())
                newRE = RE.permute( expansion_permutation )
                # out_xor = newRE
                out_xor = newRE ^ ( round_key[roundRev] )
                RE_Temp = substitute(out_xor)
                RE_modified = RE_Temp.permute( pbox_permutation )
                newRE = ( RE_modified ^ LE)
        ...
        mult+=1
        # break
        print(retString.get_bitvector_in_ascii())
        
        f.write(retString.get_bitvector_in_ascii().rstrip('\0'))
    f.close()
